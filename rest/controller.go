...
type IController interface {
	GetName() string
	Register(app *Rest, group *gin.RouterGroup)
...
func (c controllerImpl) Register(app *Rest, group *gin.RouterGroup) {
	...
	for path, handlers := range c.get {
		group.GET(path, rest2gin(handlers)...)
	}

	for path, handlers := range c.post {
		group.POST(path, rest2gin(handlers)...)
	}
...
func rest2gin(handlers []ResponseHandler) (ginHandlers []gin.HandlerFunc) {
	for _, handler := range handlers {
